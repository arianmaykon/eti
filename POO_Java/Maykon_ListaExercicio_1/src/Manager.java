...
public class Manager {
	...
	public static void main(String[] args) {
		...
		qtdAcoes = manager.scan.nextInt();
		if (qtdAcoes > 0) {
			manager.acoes = new Acao[qtdAcoes];
			while (true) {
				manager.showAndHandleMenu();
			}
		}
	}
	...
	public void showAndHandleMenu() {
		...
		switch (opcao) {
			...
			case 7:
				...
				this.consultar(this.scan.nextInt());
				break;
			case 8:
				System.exit(0);
				break;
	...
	private void cadastrarAcao() {
		if (this.indices < this.acoes.length) {
			...
			if (this.getIndice(codigo) < 0) {
				...
				acao.setDescricao(this.scan.next());
		
				System.out.println("Digite o valor de compra da Ação :");
				acao.setValorCompra(this.scan.nextDouble());
		
				System.out.println("Digite o valor de venda da Ação :");
				acao.setValorVenda(this.scan.nextDouble());
